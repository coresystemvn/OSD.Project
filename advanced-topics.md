...
```mermaid
graph TD
    A[WinPE boot] --> B{Launcher}
    B -- winpeshl.ini --> C[CoreSystem-ng.exe]
    B -- startnet.cmd --> D[CoreSystem-ng.ps1]
    C --> E[OS Configurator Dialog]
    D --> E
    E --> F[Deploy-OSDCloudCLI]
    F --> G[Installation Progress Window]
    G --> H[Inject SetupFiles]
    H --> I[Reboot & unattend.xml]
    I --> J[Post-setup.ps1]
```
