# Getting Started (Phiên bản PowerShell)
> Dành cho: Nhóm đam mê IT muốn tùy chỉnh mã nguồn

---

**Ghi chú:** Đây là lối tắt cho ai muốn nhanh nhất có đĩa boot. Dự án không chia sẻ file .iso pre-built để tránh vi phạm bản quyền. Làm theo từng bước là được. Nếu muốn hiểu sâu hơn, xem [Advanced Topics](./advanced-topics.md).
...
```bash
git clone https://github.com/coresystemvn/OSDCloud.git
cd OSDCloud
```
...
**Lưu ý:** File ISO hoàn chỉnh khoảng ~1.1GB
...
```
X:\Windows\System32\
├── coresystem-ng.ps1              ← File chính
├── startnet.cmd                   ← Launcher
...
**Lưu ý:** Quy trình khởi động WinPE:
1. WinPE khởi động → `Winpeshl.exe` được gọi
2. Nếu có `winpeshl.ini` → Đọc và gọi ứng dụng theo thứ tự
3. Nếu không có `winpeshl.ini` → Chuyển sang `startnet.cmd`

### 3.3 Chỉnh sửa startnet.cmd

File `startnet.cmd` chạy khi WinPE khởi động nhưng **không có** `winpeshl.ini`:
...
:: Dat ten cua so ung dung
title Main System (do not close this window)

:: Goi ung dung
powershell.exe coresystem-ng.ps1
```
...
├── Resources/
│   ├── coresystem-ng.ps1           ← File chính
│   ├── coresystem.ps1.old          ← Phiên bản cũ
│   ├── SetupFiles/
│   │   ├── unattend.xml
│   │   ├── post-setup-tweaks.ps1
│   │   └── post-setup-combo.ps1
...
2. **startnet.cmd** - Đã cấu hình đúng chưa?
3. **File coresystem-ng.ps1** - Có trong System32 không?
...
### Có thể tùy chỉnh phiên bản PowerShell không?

**Hoàn toàn có thể!** Đó chính là mục đích của phiên bản này.

Chỉnh sửa `coresystem-ng.ps1` và các script trong Resources/.
...
> For: IT enthusiasts who want to customize source code
...
**Note:** Final ISO file is approximately ~1.1GB
...
**Note:** WinPE boot process:
1. WinPE boots → `Winpeshl.exe` is called
2. If `winpeshl.ini` exists → Read and call applications in order
3. If no `winpeshl.ini` → Falls back to `startnet.cmd`

### 3.3 Edit startnet.cmd

The `startnet.cmd` file runs when WinPE starts but **no** `winpeshl.ini` is found:
...
├── Resources/
│   ├── coresystem-ng.ps1           ← Main file
│   ├── coresystem.ps1.old          ← Old version
│   ├── SetupFiles/
│   │   ├── unattend.xml
│   │   ├── post-setup-tweaks.ps1
│   │   └── post-setup-combo.ps1
...
2. **startnet.cmd** - Is it configured correctly?
3. **coresystem-ng.ps1 file** - Is it in System32?
...
### Can I customize the PowerShell version?

**Absolutely!** That's the purpose of this version.

Edit `coresystem-ng.ps1` and scripts in Resources/.
...
- **Repository:** https://github.com/coresystemvn/OSD.Project
